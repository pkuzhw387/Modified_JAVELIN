...
def Baldwin_scale(zydata=None, line_ind=1, norm=None, slope=None, sigma=0.0, tau=0.0, lag=0.0, alpha=1.0):
    ...
    # norm: float
    #       the norm of the Baldwin effect, 
    #       cf. ZHW's paper note for the formulation. (i.e. the cp parameter in the notes)
    # slope: float
    #        slope of the Baldwin effect, i.e. the \beta parameter in the notes
    # sigma: float
    #        used to predict the delayed continuum flux using DRW parameter
    # tau: float
    #      used to predict the delayed continuum flux using DRW parameter
    # lag: float
    #      the lag between the emission line and the continuum
    # alpha: float
    ...
    if zydata is None or norm is None or slope is None:
        ...
        # a = raw_input("just a input to stop system...")
        return alpha * norm * (zydata.mlist[0])**(1 + slope) / delayed_cont_flux

def unpackphotopar(p, zydata=None, nlc=2, hascontlag=False, GPmodel="DRW", baldwin=False):
    """ Unpack the physical parameters from input 1-d array for photo mode.

    baldwin: bool, to specify whether to take Baldwin effect into account.
    Note if baldwin == True, the lscales only includes the line scales but
    not scale_hidden, and the shape also changes from (2*nline, 1) to (nline, len(zydata.jlist[0]))
    ...
    if baldwin == False:
        ...
        llags = np.zeros(2 * nlc - 2)
        lwids = np.zeros(2 * nlc - 2)
        lscales = np.ones(2 * nlc - 2)

        if hascontlag:
            for i in range(len(llags) / 2):
                llags[2 * i] = p[2 + 6 * i]
                lwids[2 * i] = p[3 + 6 * i]
                lscales[2 * i] = p[4 + 6 * i]
                # contlag handling
                llags[2 * i + 1] = p[5 + 6 * i]
                lwids[2 * i + 1] = p[6 + 6 * i]
                lscales[2 * i + 1] = p[7 + 6 * i]
        else:
            for i in range(len(llags) / 2):
                llags[2 * i] = p[2 + 4 * i]
                lwids[2 * i] = p[3 + 4 * i]
                lscales[2 * i] = p[4 + 4 * i]
                # since the cont in the line+cont band has zero lag and zero width,
                # we only need to consider its scale.
                lscales[2 * i + 1] = p[5 + 4 * i]
        ...
        if zydata is None:
            raise InputError(msg="Need zydata to calculate the line scales!")
        else:
            llags = np.zeros(2 * nlc - 2)
            lwids = np.zeros(2 * nlc - 2)
            lscales = np.zeros((nlc - 1, len(zydata.jlist[0])))
            ...
            for i in range(len(llags) / 2):
                if hascontlag:
                    ...
                    llags[2 * i] = p[2 + 5 * i]
                    lwids[2 * i] = p[3 + 5 * i]
                    lscales[i] = Baldwin_scale(zydata=zydata, line_ind=i, norm=p[4 + 5 * i], slope=p[5 + 5 * i], \
                                               sigma=sigma, tau=tau, lag=llags[2 * i], alpha=p[6 + 5 * i])

        # scale_hidden = [p[6 + 5 * i] for i in range(nlc - 1)]
        # I decided to include the pure continuum scale_hidden (1.0) in this array.
        scale_hidden = [1.0]
        for i in range(nlc - 1):
            scale_hidden.append(p[6 + 5 * i])
        scale_hidden = np.array(scale_hidden)

        if GPmodel == "pow-law":
            return(A, gamma, llags, lwids, lscales, scale_hidden)
        elif GPmodel == "DRW":
            return(sigma, tau, llags, lwids, lscales, scale_hidden)
...
def lnpostfn_photo_p(p, zydata, GPmodel="DRW", hascontlag=False, conthpd=None, ratiohpd=None, set_extraprior=False,
    ...
    ratiohpd: ndarray, optional
        Priors on line_scale and alpha. with shape (3, 2) or (1, 2).
        if the shape is (3, 2), the values are the following:
        np.array([[line_scale_low, alpha_low],
                  [line_scale_med, alpha_med],
                  [line_scale_hig, alpha_hig]])
        otherwise:
        np.array([line_scale0, alpha0]), where the log(line_scale0) is the central value,
        and the upper limit is 2*line_scale0, lower limit is 0.5*line_scale0, the same for
        alpha.
    ...
    baldwin: bool, optional
        to specify whether use the baldwin effect to calculate the line scales
    ...
    if fixed is not None:
        ...
        p = np.asarray(p)
        p = p * fixed + p_fix * (1.0 - fixed)

    if baldwin == True:
        try:
            if GPmodel == "pow-law":
                A, gamma, llags, lwids, lscales, scale_hidden = unpackphotopar(p, zydata, zydata.nlc,hascontlag=hascontlag, GPmodel="pow-law", baldwin=True)
                ...
                sigma, tau, llags, lwids, lscales, scale_hidden = unpackphotopar(p, zydata, zydata.nlc,hascontlag=hascontlag, GPmodel='DRW', baldwin=True)
        except ValueError:
            return -np.inf
    ...
    #     # print ratiohpd
    #     if np.shape(ratiohpd) == (2,):
    #         ratiohpd = np.array([[0.5 * ratiohpd[0], 0.5 * ratiohpd[1]],\
    #                              [ratiohpd[0], ratiohpd[1]],\
    #                              [2 * ratiohpd[0], 2 * ratiohpd[1]]])
    #     # print ratiohpd
    #     if np.shape(ratiohpd) == (3, 2):
    #         # for line_scale
    #         if p[4] < ratiohpd[1, 0]:
    #             # reject too small scales
    #             if p[4] < ratiohpd[0, 0]:
    #                 prior4 = my_pos_inf
    #             else:
    #                 prior4 = (np.log(p[4]) - np.log(ratiohpd[1,0]))/(np.log(ratiohpd[1,0])-np.log(ratiohpd[0,0]))
    #         else:
    #             if p[4] > ratiohpd[2, 0]:
    #                 prior4 = my_pos_inf
    #             else:
    #                 prior4 = (np.log(p[4]) - np.log(ratiohpd[1,0]))/(np.log(ratiohpd[2,0])-np.log(ratiohpd[1,0]))

    #         if p[5] < ratiohpd[1, 1]:
    #             if p[5] < ratiohpd[0, 1]:
    #                 prior5 = my_pos_inf
    #             else:
    #                 prior5 = (np.log(p[5]) - np.log(ratiohpd[1,1]))/(np.log(ratiohpd[1,1])-np.log(ratiohpd[0,1]))
    #         else:
    #             if p[5] > ratiohpd[2, 1]:
    #                 prior5 = my_pos_inf
    #             else:
    #                 prior5 = (np.log(p[5]) - np.log(ratiohpd[1,1]))/(np.log(ratiohpd[2,1])-np.log(ratiohpd[1,1]))

    # else:
    #     prior4 = 0.0
    #     prior5 = 0.0
        # print "no prior on alpha and line scale."
    ...
    if widtobaseline < 1.0:
        for i in range(zydata.nlc - 1):
            if np.abs(lwids[2 * i]) > widtobaseline*zydata.rj:
                prior2 += np.log(np.abs(lwids[2 * i])/(widtobaseline*zydata.rj))
    if widlimit is not None:
        for i in range(zydata.nlc - 1):
            if lwids[2 * i] > widlimit[0][1] or lwids[2 * i] < widlimit[0][0]:
                prior2 += my_pos_inf
    ...
    if set_extraprior:
        ...
        for i in range(zydata.nlc - 1):
            if (np.abs(llags[2 * i]) <= zydata.cont_cad or
                    np.abs(llags[2 * i]) <= np.abs(lwids[2 * i])):
                prior2 += my_pos_inf
            # penalize on extremely small line responses (below mean error level).
            if sigma * np.abs(np.max(lscales[i])) <= np.mean(zydata.elist[i + 1]):
                prior2 += my_pos_inf
        # }}}
    # add logp of all the priors
...
def lnlikefn_photo(zydata, sigma, tau, llags, lwids, lscales, scale_hidden, set_retq=False,
    ...
    lags = np.zeros(2 * zydata.nlc - 1)
    wids = np.zeros(2 * zydata.nlc - 1)
...
class Pmap_Model(object):
	...
	GPmodel: str.
        ...
        linename: list of str, optional
            Name(s) of the emission line (default: 'line').
        ...
        if zydata is None:
            ...
            if self.baldwin:
                self.ndim = 5 * self.nlc - 3
                for i in range(len(linename)):
                    self.vars.append("_".join(["lag", linename[i]]))
                    self.vars.append("_".join(["wid", linename[i]])) 
                    self.vars.append("_".join(["cp", linename[i]])) 
                    self.vars.append("_".join(["beta", linename[i]])) #self.vars:["sigma", "tau", "lag_linename", "wid_linename", "scale_linename"]
                    self.texs.append("".join([r"$t_{", linename[i], r"}$"]))
                    self.texs.append("".join([r"$wid_{", linename[i], r"}$"]))
                    self.texs.append("".join([r"$cp_{", linename[i], r"}$"]))
                    self.texs.append("".join([r"$b_{", linename[i], r"}$"]))
                ...
                if self.hascontlag:
                    ...
                    self.texs.append("_".join([r"$t_{cont}$", linename[i]]))
                    self.vars.append("cont_wid_%s" %linename[i])
                    self.texs.append("_".join([r"$w_{cont}$", linename[i]]))
    ...
    def do_mcmc(self, conthpd=None, ratiohpd=None, set_extraprior=False, lagtobaseline=0.3, lagpenaled=None,
