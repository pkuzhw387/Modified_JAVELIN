...
def point_eval(M, C, x):
    ...
    # step-by-step description of intermediate results of the mve_var() for baldwin_scale() function
    # x_.shape = (nepoch,) -> x_.shape[0] = nepoch
    # C is a FullRankCovariance, so y_size == 1
    # since chunksize == 1e8, n_chunks == 1
    # bounds == array([0, nepoch])
    # cmin == array([0]), cmax == array([nepoch])
    # the for-loop executes only once, in which (cmin, cmax) == (0, 25)
    # so x__ == x_ == jwant
    # in M() function, regularize == False, (self.observed and observed) == True. TBC.
