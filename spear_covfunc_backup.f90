...
if (symm) then
    do j = cmin+1,cmax
        ...
        do i=1,j
            ...
            swid1 = swidarr(id1(i))
            scale1=scalearr(id1(i))
            call covmatpmapij2(mat(i,j), id1(i),id2(j),jd1(i),jd2(j),A,gama,slag1,swid1,scale1,slag2,swid2,scale2,scale_hidden)
        enddo
    enddo
else
    do j = cmin+1,cmax
...
SUBROUTINE covmatpmapij2(covij,id1,id2,jd1,jd2,A,gama,slag1,swid1,scale1,slag2,swid2,scale2,scale_hidden)
implicit none
REAL(kind=8),intent(out) :: covij
INTEGER(kind=4),intent(in) :: id1,id2
REAL(kind=8),intent(in)  :: jd1,jd2
...
if (imin .eq. imax) then
    ...
    if (imin .eq. 1) then
        ...
        ! referring to the line properties.
        ! cov(c1i,c1j)
        covij = getcmat_delta2(id1,id2,jd1,jd2,gama,0.0D0,scale_hidden,0.0D0,scale_hidden)
        ! cov(c1i, lj)
        if(swid2 .le. 0.01D0) then
            ! when swid is small, no convulted covariance is needed.
            covij = covij + getcmat_delta2(id1,id2,jd1,jd2,gama,0.0D0,scale_hidden,slag2,scale2)
        else
            !covij = getcmat_lauto(id1,jd1,jd2,tau,slag1,swid1,scale1)
            covij = covij + getcmat_lc2(id1,id2,jd1,jd2,gama,0.0D0,0.0D0,scale_hidden,slag2,swid2,scale2)
        endif
        ! cov(li, c1j)
        ! similar to cov(cli, lj), simple mutation of indices.
        if(swid1 .le. 0.01D0) then
            covij = covij + getcmat_delta2(id1,id2,jd1,jd2,gama,slag1,scale1,0.0D0,scale_hidden)
        else
            covij = covij + getcmat_lc2(id1,id2,jd1,jd2,gama,slag1,swid1,scale1,0.0D0,0.0D0,scale_hidden)
        endif
        ! cov(li, lj)
        if(swid1 .le. 0.01D0) then
    ...
    ! continuum band and line band cross cov(c0i, c1j) + cov(c0i, lj)
    ! cov(c0i, c1j)
    covij = getcmat_delta2(id1,id2,jd1,jd2,gama,0.0D0,1.0D0,0.0D0,scale_hidden)
    ! cov(c0i, lj)
    twidth = max(swid1, swid2)
    if (twidth .le. 0.01D0) then
        covij = covij + getcmat_delta2(id1,id2,jd1,jd2,gama,slag1,scale1,slag2,scale2)
    else
        covij = covij + getcmat_lc2(id1,id2,jd1,jd2,gama,slag1,swid1,scale1,slag2,swid2,scale2)
    endif
endif
