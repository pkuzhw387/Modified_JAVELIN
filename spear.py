...
def spear_threading2(x,y,idx,idy,A,gamma,lags,wids,scales,symm=None,set_pmap=False,blocksize=10000) :
    """
    threaded version, divide matrix into subblocks with *blocksize*
    elements each. Do not use it when multiprocessing is on (e.g., in emcee MCMC
    sampling).
    ...
    if set_pmap :
        def targ(C,x,y,idx,idy,cmin,cmax,symm) :
            SCF.covmatpmap_bit(C,x,y,idx,idy,A,gamma,lags,wids,scales,cmin,cmax,symm)
    else :
        def targ(C,x,y,idx,idy,cmin,cmax,symm) :
...
def spear2(x,y,idx,idy,A,gamma,lags,wids,scales,symm=None,set_pmap=False) :
    """ Clean version without multithreading. Used when multiprocessing is on
    (e.g., in emcee MCMC sampling).
    ...
    C = np.asmatrix(np.empty((nx,ny),dtype=float,order='F'))
    if set_pmap :
        SCF.covmatpmap_bit2(C,x,y,idx,idy,A,gamma,lags,wids,scales,0,-1,symm)
    else :
